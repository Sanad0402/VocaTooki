...
def _prune_generated_tests():
    ...
    try:
        result = _fresh_generator().prune_to_suite()
    except Exception:
...
def _fresh_generator():
    """A RallyTestGenerator built from the CODE ON DISK, not the one imported
    at boot.

    The panel serves with no reloader, so a long-running process keeps whatever
    version of the generator it started with. That has bitten three times: a
    case regenerated by an old generator came out as a passing "open the page"
    test twice, and a sync rewrote a test identically and so reported "nothing
    changed" when the generator had in fact been fixed. Reloading here means
    editing the generator and pressing Sync is enough.
    """
    import importlib
    from runner import test_generator as _tg
    importlib.reload(_tg)
    return _tg.RallyTestGenerator(_ROOT)
...
def api_case_generate(tc_id):
    ...
    if path is None:
        try:
            path = _fresh_generator().generate_test(tc)
        except Exception as e:
