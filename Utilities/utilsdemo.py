...
# The backend — which VocaTooki host every API call goes to, the class map and
# the user state — lives in Utilities/vt/backend.py. Its RUN-TIME values
# (VT_DATA_API, VT_TASKS_API, _VT_BACKEND) change when a run picks an API, so
# they are never copied into this module: code here reads `_backend.<name>`, and
# `utilsdemo.<name>` from outside is served live by __getattr__ at the bottom.
from Utilities.vt import backend as _backend  # noqa: E402
from Utilities.vt.backend import (  # noqa: E402,F401
    VT_BACKEND_AUTO, VT_BACKENDS, _VT_DATA_API_DEFAULT, _VT_DATA_API_HOSTS_DEFAULT,
    _VT_ENV_AT_IMPORT, _VT_TASKS_API_DEFAULT, _CLASS_MAP_HOST,
    current_backend, backend_pinned, set_backend, _vt_data_bases,
    get_class_map, get_user_state,
)
...
def class_tasks(class_id, token=None):
    ...
    try:
        r = requests.get(f"{_backend.VT_TASKS_API}/get-class-tasks/{class_id}",
...
def user_class_id(user_id, token=None):
    ...
    """
    try:
        r = requests.get(f"{_backend.VT_TASKS_API}/get-user-tasks/{user_id}",
                         headers=task_api_headers(token), timeout=25)
        r.raise_for_status()
...
def task_answer_key(task_id, token=None):
    ...
    """
    try:
        r = requests.get(f"{_backend.VT_TASKS_API}/get-task/{task_id}",
                         headers=task_api_headers(token), timeout=25)
        r.raise_for_status()
        task = r.json() or {}
    except Exception as e:                           # noqa: BLE001
...
def task_submitted_answers(user_id, task_id, token=None, attempt=0):
    ...
    try:
        r = requests.get(
            f"{_backend.VT_TASKS_API}/get-task-answer/{user_id}/{task_id}/{attempt}",
            headers=task_api_headers(token), timeout=25)
        r.raise_for_status()
        return r.json() or {}
    except Exception as e:                           # noqa: BLE001
    ...
    if username and password:
        ...
        if who.get("user_id"):
            ...
            if who.get("backend") and not backend_pinned():
                _backend.VT_TASKS_API = f"{who['backend']}/data"
        elif not user_id:
...
def get_user_exam_by_userid_examid(userid_examid, class_id=2336, token=None, username=None, password=None):
    ...
    url = f"{_backend.VT_DATA_API}/get-user-exams/{user_id}/{class_id}"
...
# taken at import would keep pointing at the API chosen before the run.
_LIVE_NAMES = {"VT_DATA_API": _backend, "VT_TASKS_API": _backend, "_VT_BACKEND": _backend}


def __getattr__(name):
    """utilsdemo.<name> for a value that lives in a vt module (PEP 562)."""
