...
def _utils():
    from Utilities import utilsdemo            # local: utilsdemo installs this module
    return utilsdemo
...
def _clear_once(driver):
    ...
    u = _utils()
    acted = bool(u.dismiss_screen_blocker(driver, tries=1, settle=0.3))
    ...
    if u.parrot_bubble_shown(driver) is True:
        ...
        # it (user, 2026-09-22).
        acted = bool(u.dismiss_help_popup(driver, allow_tap=True)) or acted
    return acted
...
def on_scene_entered(driver, scene=None, built=False):
    ...
    building (``utilsdemo.wait_for_scene`` does), so it is not waited for twice.
    Never raises.
    """
    ...
    try:
        u = _utils()
        scene = scene or u._current_scene(driver)
        st = _state.setdefault(id(driver), {"scene": None, "checked": 0.0})
        st["scene"] = scene
        if not built:
            u.wait_for_scene_ready(driver, label=scene or "scene")
