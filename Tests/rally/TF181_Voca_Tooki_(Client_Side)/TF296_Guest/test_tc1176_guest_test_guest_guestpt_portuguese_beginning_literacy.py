"""
TC1176 — Guest - test guest guestPt - Portuguese - Beginning Literacy

Auto-generated from Rally (Method = Automated).

Description:
    Guest Flow Test — Portuguese / Beginning Literacy Verifies that a guest user can complete the registration flow, enter the map, access activities, solve the first exam, and that levels greater than 5 are locked in guest mode. Language: Portuguese Difficulty: Beginning Literacy Avatar Gender: Male Test Type: Functional | Priority: High | Severity: Major

(No test steps recorded in Rally — add them to the case, then re-sync.)

Validation (from Rally):
    Input:    1. Launch the app and tap Start Free Trial button. 2. Tap Let's Start button. 3. Enter test guest in the First Name field. 4. Enter guestPt in the Last Name field. 5. In the 'Choose Your Native Language' screen, select Portuguese. 6. Tap Next. 7. On the difficulty selection screen, select Beginning Literacy. 8. Wait for the 'You Are' (gender) screen to appear, then select Male. 9. On the avatar customization screen, tap the Back button. 10. Tap anywhere on screen to dismiss the parrot. 11. Enter the map. 12. ✅ Solve at least one activity in each accessible level (activities must be solved once per language). 13. Navigate to the first exam on the map and complete it. 14. After the exam, tap on any level greater than 5 — verify it is locked (guest restriction). 15. Perform Clear Data to reset app state for the next test.
    Expected: - Guest registration completes successfully with correct name and language. - Difficulty 'Beginning Literacy' is correctly applied. - Avatar gender 'Male' is selected and displayed correctly. - Map loads; levels 1–5 are accessible. - Activities launch and run correctly inside map levels. - First exam is accessible and completable. - Levels greater than 5 display a lock icon and cannot be entered (guest restriction enforced). - Clear Data resets app to the login/start screen.
...
# identified afterwards by what it selected (Rally said "guestPt").
FIRST_NAME = "test guest"
LAST_NAME = "guest3rdPo"
# What this case picks on the onboarding option screens, in any order.
OPTIONS = ["Portuguese", "Beginning Literacy", "Male"]
...
def test_tc1176_guest_test_guest_guestpt_portuguese_beginning_literacy(altdriver):
    ...
    assert result["ok"], (
        ...
        f"Expected: - Guest registration completes successfully with correct name and language. - Difficulty 'Beginning Literacy' is correctly applied. - Avatar gender 'Male' is selected and displayed correctly. - Map lo...")
    ...
    # 6. The guest restriction: a level past the accessible band must not
    ...
    # 7. Rally steps that cannot be derived from the description — implement
    #    against the live app, then set MANUAL_EDIT = True to keep the code:
    #   1. Enter the map.
    #   2. ✅ Solve at least one activity in each accessible level (activities must be solved once per language).
    #   3. Navigate to the first exam on the map and complete it.
    #   4. After the exam, tap on any level greater than 5 — verify it is locked (guest restriction).
    #   5. Perform Clear Data to reset app state for the next test.

    # 8. Reset for the next run (Rally: "Perform Clear Data"). Order matters:
