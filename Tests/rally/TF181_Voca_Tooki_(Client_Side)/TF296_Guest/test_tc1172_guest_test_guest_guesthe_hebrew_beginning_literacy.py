"""
TC1172 — Guest - test guest guestHe - Hebrew - Beginning Literacy

Auto-generated from Rally (Method = Automated).

Description:
    Guest Flow Test — Hebrew / Beginning Literacy Verifies that a guest user can complete the registration flow, enter the map, access activities, solve the first exam, and that levels greater than 5 are locked in guest mode. Language: Hebrew Difficulty: Beginning Literacy Avatar Gender: Male Test Type: Functional | Priority: High | Severity: Major

(No test steps recorded in Rally — add them to the case, then re-sync.)

Validation (from Rally):
    Input:    1. Launch the app and tap Start Free Trial button. 2. Tap Let's Start button. 3. Enter test guest in the First Name field. 4. Enter guestHe in the Last Name field. 5. In the 'Choose Your Native Language' screen, select Hebrew. 6. Tap Next. 7. On the difficulty selection screen, select Beginning Literacy. 8. Wait for the 'You Are' (gender) screen to appear, then select Male. 9. On the avatar customization screen, tap the Back button. 10. Tap anywhere on screen to dismiss the parrot. 11. Enter the map. 12. ✅ Solve at least one activity in each accessible level (activities must be solved once per language). 13. Navigate to the first exam on the map and complete it. 14. After the exam, tap on any level greater than 5 — verify it is locked (guest restriction). 15. Perform Clear Data to reset app state for the next test.
...
# identified afterwards by what it selected (Rally said "guestHe").
FIRST_NAME = "test guest"
LAST_NAME = "guest3rdHe"
# What this case picks on the onboarding option screens, in any order.
OPTIONS = ["Hebrew", "Beginning Literacy", "Male"]
...
def test_tc1172_guest_test_guest_guesthe_hebrew_beginning_literacy(altdriver):
